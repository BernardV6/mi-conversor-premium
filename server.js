...
if (process.env.ADMIN_USER && process.env.ADMIN_PASS) {
    admins[process.env.ADMIN_USER] = { password: process.env.ADMIN_PASS };
} else {
    console.warn('[SISTEMA] Admin credentials no configuradas; rutas de admin deshabilitadas.');
}
...
const adminAuth = (req, res, next) => {
    ...
    if (!user || !admins[user.name] || admins[user.name].password !== user.pass) {
        ...
        console.warn(`[!] Intento fallido de admin desde IP: ${req.ip || req.connection.remoteAddress}`);
...
const premiumUsers = new Set(); // Simulacro de DB para usuarios premium (usar Set para búsquedas rápidas)

// Rate limiter (24h; skip para usuarios premium)
const limiter = rateLimit({
    ...
    skip: (req) => {
        const id = req.ip || req.connection.remoteAddress;
        return premiumUsers.has(id);
    }
});

// Rate limiter moved below premiumUsers so we can `skip` premium users dynamically.
...
const upload = multer({
    storage: storage,
    limits: {
        fileSize: config.premiumMaxSize * 1024 * 1024 // Límite global = tamaño máximo premium; se validará por usuario abajo
    },
...
const isPremiumUser = (req) => {
    ...
    return premiumUsers.has(userIdentifier);
};
...
app.post('/stripe-webhook', express.raw({ type: 'application/json' }), (req, res) => {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        if (premiumIdentifier && !premiumUsers.has(premiumIdentifier)) {
            premiumUsers.add(premiumIdentifier);
...
app.post('/convert', limiter, upload.single('file'), (req, res) => {
    if (!req.file) return res.status(400).json({ error: 'Archivo inválido.' });

    const userIP = req.ip || req.connection.remoteAddress;
    const stats = getUserStats(userIP);
    const isPremium = premiumUsers.has(userIP);

    if (!isPremium && stats.conversions >= config.freeLimit) {
        if (fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);
        return res.status(429).json({ error: 'Ha alcanzado su límite diario de conversiones. Actualiza a Premium.' });
    }

    const maxAllowed = (isPremium ? config.premiumMaxSize : config.freeMaxSize) * 1024 * 1024;
    if (req.file.size > maxAllowed) {
        if (fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);
        return res.status(400).json({ error: 'El archivo excede el límite de su plan.' });
    }

    const outputPath = path.join(outputsDir, `converted-${Date.now()}.avi`);
    console.log(`[FFMPEG] Iniciando conversión para ${userIP} -> ${req.file.path}`);
    ...
    ffmpeg.on('error', err => {
        console.error(`[FFMPEG] Error al iniciar ffmpeg: ${err.message}`);
        if (fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);
        return res.status(500).json({ error: 'Error al iniciar el motor de conversión.' });
    });
    ...
    ffmpeg.on('close', code => {
        if (code === 0 && fs.existsSync(outputPath)) {
            ...
            res.download(outputPath, (err) => {
                // borrar input y output
                try { if (fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path); } catch(e) {}
                try { if (fs.existsSync(outputPath)) fs.unlinkSync(outputPath); } catch(e) {}
                if (err) console.error('[DOWNLOAD] Error al enviar archivo:', err);
            });
        } else {
            try { if (fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path); } catch(e) {}
            try { if (fs.existsSync(outputPath)) fs.unlinkSync(outputPath); } catch(e) {}
            console.error('[FFMPEG] Conversión fallida, código:', code);
            return res.status(500).json({ error: 'Error en el motor de conversión.' });
        }
    });
});
